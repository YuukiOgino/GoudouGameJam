...
public class HandController : MonoBehaviour {
	...
	// 前のハンドX位置
	private float beforeHandX = 0.0f;
		...
		for (int h = 0; h < num_hands; ++h) {
			...
			if (model != null) {
				...
				else 
				{
					...
					// print(leap_hand.PalmPosition.x.ToString());
					float moveX = beforeHandX - leap_hand.PalmPosition.x;
				...
				// ハンドXをメモリに保存
				beforeHandX = leap_hand.PalmPosition.x;
			}
		}
